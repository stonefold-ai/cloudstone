...
package ai.stonefold.cloudstone.server.config;

import ai.stonefold.cloudstone.runtime.TenantId;
import ai.stonefold.cloudstone.server.web.TenantAuthFilter;
import ai.stonefold.cloudstone.tenancy.ApiKeys;
import ai.stonefold.cloudstone.tenancy.InMemoryTenantRegistry;
import ai.stonefold.cloudstone.tenancy.Tenant;
import ai.stonefold.cloudstone.tenancy.TenantRegistry;
...
public class GatewayConfig {
     ...
     *
     * An empty binding can also mean the whole cloudstone.tenants block is
    ...
    static InMemoryTenantRegistry buildRegistry(TenantSeedProperties seeds) {
        ...
        for (TenantSeedProperties.TenantSeed seed : seeds.tenants()) {
            if (!ids.add(seed.id())) {
                throw new IllegalStateException(
                        "duplicate tenant id '" + seed.id() + "' in cloudstone.tenants");
            }
        ...
        if (seeds.tenants().isEmpty()) {
            ...
            log.warn("""
                    ...
                    If you DID configure tenants, your cloudstone.tenants block is not where
