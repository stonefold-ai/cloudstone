...
class TenantSeedBindingTest {
    ...
    static void seedProperties(DynamicPropertyRegistry registry) {
        registry.add("cloudstone.tenants[0].id", () -> "acme");
        registry.add("cloudstone.tenants[0].display-name", () -> "Acme Corp");
        registry.add("cloudstone.tenants[0].key-hashes[0]", () -> ApiKeys.hash(KEY));
    }
